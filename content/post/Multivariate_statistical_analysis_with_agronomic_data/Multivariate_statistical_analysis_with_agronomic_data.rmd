...
In this exercise, I will be analyzing agronomic datasets from four fields. Using this data and we try to investigate the effects of variable seeding rates in overall yield.

The top row is target seeding rates, in seeds per acre, the bottom row is
...
calculate_field_mean_SD_and_get_RR_EffSize <- 
  function(field, intervals, Single.Field.Analysis = TRUE, printplot = FALSE){
...
I have some additional functions for this project written here.
```{r}
...
different from what I found in `Part 1`. I analyzed these data for Date
