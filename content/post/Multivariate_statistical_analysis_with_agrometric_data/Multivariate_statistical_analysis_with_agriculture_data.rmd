...
title: "Multivariate statistical analysis with agrometric data"
...
statistical consultant. This data comes from four different farm fields and we try to investigate the effects of variable seeding rates in overall yield.
...
1. First you have to define the path to all csv files for part2 under (`path.part2`) and all .csv files from part1 under `path.part1` in the begining of the code below.
...
First of all we, will set two directory paths where we have all our files we want to anlalyze. first path is `path.part2` for part2 data and `path.part1` for part1 data
...
# # Path to all .csv data files for part2.
...
Additionally, let's read our csv files from part1
...
print(paste0("Read file ", all.files[i], ' !'))
  ...
  # From part1 for fieldB, we saw significant difference between all Control Rates of
  ...
  # From part1 for fieldC, we saw all Control rates of seeding are significantly diffferent with the highest Mean yield for 28000.
...
```  
  ...
  Based on my analysis from part1, we saw that it is better to compare all
...
difference to  what I found for my part1. I analyzed these data for Date
effect also and it looks like there is no Date effect on Mean Yield. I thought
the skewned in 28000 interval from my part1 would be addressed by Date effect,
