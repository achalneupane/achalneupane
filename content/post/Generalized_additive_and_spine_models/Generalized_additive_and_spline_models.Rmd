...
Here, we will use the body fat data ($\textbf{ bodyfat}$ data from $\textbf{TH.data}$  package) for some analysis. 

First, we will explore the data graphically to see what variables need to be included for predicting bodyfat. We can check for the correlated predictors.
...
We then fit a generalized additive model assuming normal errors. 
...
plot(bodyfat_gam)
```
Based on this plot, it looks like we don't need to smooth all variables. Variables age, waistcirc, elbowbreadth & anthro3a appear linear and don't need smoothing.

Question: How do we evaluate a covariate's 'informative'-ness?
Answer: Look at the p-values of the covariates.
...
From `gam.check`, While the residuals plot appears random, the histogram makes the data appear skewed to the left, so the assumption of normality doesn’t entirely hold.
...
Now we will remove the insignificant variables. We will also remove smoothing for some variables and report the summary, plot, GCV, AIC, adj-R$^2$.
